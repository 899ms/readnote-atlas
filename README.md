...
- Desktop caption transport is built into Readnote Atlas—no second Chrome extension. With the [local macOS helper](scripts/desktop-captions/README.md) running, one resizable always-on-top panel displays bilingual captions while you use other tabs/apps or minimize Chrome. Returning to YouTube hides it; pausing retains it. Hover reveals ±15-second seek, play/pause, and bookmark controls. The helper must be installed/run separately from Chrome; the source setup does not install an auto-start service. Without it, the existing browser PiP path remains available subject to Chrome permissions and limitations.
