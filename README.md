...
- Background captions default to Auto: register Chrome's Media Session `enterpictureinpicture` action when a YouTube video loads, without clicking Enable. On eligible Chrome 134+ installations, the browser can open bilingual Document Picture-in-Picture captions when leaving an audibly playing video. Returning to YouTube closes the window. Chrome may request automatic PiP permission; recent audible playback, audio focus, site engagement/permission, and no conflicting PiP window are browser prerequisites. Unsupported browsers retain a manual Open fallback. Auto can be switched Off in the collapsed caption settings. Chrome still owns the outer frame and source label; this is not a borderless desktop overlay. Minimizing Chrome and switching desktop apps require testing on the target OS and are not guaranteed by this tab-switch integration.
