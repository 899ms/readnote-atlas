...
function normalizeCaptionText(text) {
  return ReadnoteTranscript.normalizeText(text);
}
...
function groupTranscriptEntries(entries, limits = TRANSCRIPT_SEGMENT_LIMITS) {
  return ReadnoteTranscript.groupEntries(entries, limits).map((segment) => ({
    ...segment,
    texts: [segment.text],
  }));
}
 ...
 */
function knowledgeSyncLabel(note) {
  const sync = note?.knowledgeSync || {};
  if (sync.status === "syncing") return "Saving to knowledge base…";
  if (sync.status === "synced") {
    if (sync.obsidian === "synced" && sync.notion === "synced") {
      return "Obsidian + Notion";
    }
    if (sync.obsidian === "synced") return "Saved to Obsidian";
    if (sync.notion === "synced") return "Saved to Notion";
  }
  if (sync.status === "not_configured") return "Knowledge base not configured";
...
function renderNotes(notes, filteredVideoId) {
  ...
  notes.forEach((note, index) => {
    ...
    noteEl.querySelector(".note-sync").addEventListener("click", async () => {
      const button = noteEl.querySelector(".note-sync");
      button.disabled = true;
      button.textContent = "Syncing…";
      try {
        await chrome.runtime.sendMessage({ action: "syncNote", noteId: note.id });
